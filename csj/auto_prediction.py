...
from utils.config import polypfive,isic2018, duts,COD10K,SBU,trans10k_easy,trans10k_hard,trans10k_all,covid,breast
from utils.misc import check_mkdir
from model.DBS_group_prompt import Spider_fast_prompt_infer, Spider_fast_prompt_infer_one_encoder
# from model.models import Spider_fast_prompt_infer_one_encoder
from model.model_dino import Spider_fast_prompt_infer_dino_one_encoder
...
# exp_name = 'Spider_swinb_one_encoder_1cuda'
exp_name = 'Spider-Swin-Base-Pre'
args = {
...
image_sod_root = "datasets/DUTS/DUTS-TR/DUTS-TR-Image"
image_cod_root = "datasets/COD10K/TrainDataset/TrainDataset/Imgs"
image_shadow_root = "datasets/SBU-shadow/SBUTrain4KRecoveredSmall/images"
image_transparent_root = "datasets/Trans10K/train/images"
image_polyp_root = "datasets/unified_lesion_dataset/polyp/TrainDataset/TrainDataset/image"
image_covid_root = "datasets/unified_lesion_dataset/COVID-19_Lung_Infection_train/images"
image_breast_root = "datasets/unified_lesion_dataset/breast/images"
image_skin_root = "datasets/unified_lesion_dataset/isic2018/train/images"


gt_sod_root = "datasets/DUTS/DUTS-TR/DUTS-TR-Mask"
gt_cod_root = "datasets/COD10K/TrainDataset/TrainDataset/GT"
gt_shadow_root = "datasets/SBU-shadow/SBUTrain4KRecoveredSmall/labels"
gt_transparent_root = "datasets/Trans10K/train/mask_binary"  # mask_binary
gt_polyp_root = "datasets/unified_lesion_dataset/polyp/TrainDataset/TrainDataset/mask"
gt_covid_root = "datasets/unified_lesion_dataset/COVID-19_Lung_Infection_train/masks"
gt_breast_root = "datasets/unified_lesion_dataset/breast/masks"
gt_skin_root = "datasets/unified_lesion_dataset/isic2018/train/masks"
...
# to_test = {'all':trans10k_all}  #{'easy':trans10k_easy,'hard':trans10k_hard} # images, mask_binary
...
data_to_test = [{'DUTS': duts}, {'COD10K': COD10K}, {'SBU': SBU}, {'all': trans10k_easy},
...
def main():
    t0 = time.time()
    net = Spider_fast_prompt_infer().cuda()#Spider_fast_prompt_infer_one_encoder().cuda()
    ...
    with torch.no_grad():
        for to_test in data_to_test:
            for name, root in to_test.items():
                ...
                test_image_size = 384
                test_loader = test_get_loader(root1, batchsize=64,trainsize=test_image_size)
                if name == 'DUTS':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_sod_root, gt_sod_root, sod_prompt, batchsize=64, trainsize=test_image_size)
                elif name == 'COD10K':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_cod_root, gt_cod_root, cod_prompt, batchsize=64, trainsize=test_image_size)
                elif name == 'SBU':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_shadow_root, gt_shadow_root, shadow_prompt, batchsize=64, trainsize=test_image_size)
                elif name == 'all':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_transparent_root, gt_transparent_root, transparent_prompt, batchsize=64, trainsize=test_image_size)
                elif name == 'polyp':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_polyp_root, gt_polyp_root, polyp_prompt, batchsize=64, trainsize=test_image_size)
                elif name == 'COVID':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_covid_root, gt_covid_root,covid_prompt, batchsize=64, trainsize=test_image_size)
                elif name == 'Breast':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_breast_root, gt_breast_root, breast_prompt, batchsize=64, trainsize=test_image_size)
                elif name == 'ISIC2018':
                    train_sod_loader = image_prompt_get_loader_kmenas_choose(image_skin_root, gt_skin_root, skin_prompt, batchsize=64, trainsize=test_image_size)
                ...
                for i , (image_sod, gt_sod) in enumerate(train_sod_loader):
                    ...
                    for i,  (img, gt, img_name, w_, h_) in enumerate(test_loader):
                        ...
                        check_mkdir(os.path.join(ckpt_path, exp_name, args['snapshot'] + 'epoch', task, name))
                        for j in range(n):
                            for k in range(len(filter_list)):
                                ...
                                print(name, ': ', img_name[j])
                                cv2.imwrite(
                                    os.path.join(ckpt_path, exp_name, args['snapshot'] + 'epoch', task, name, img_name[j][:-4] + '.png'), res)

if __name__ == '__main__':
    # snapshot = ['Model_50_gen', 'Model_45_gen', 'Model_40_gen', 'Model_35_gen', 'Model_30_gen',
                # 'Model_25_gen', 'Model_20_gen', 'Model_15_gen', 'Model_10_gen', 'Model_5_gen']
    snapshot = ['Model_50_gen']
    for epo in snapshot:
        args['snapshot'] = epo
        main()
